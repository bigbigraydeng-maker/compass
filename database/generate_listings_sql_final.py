...
def generate_sql():
    ...
    sql_path = 'import_listings_final.sql'
    ...
    with open(sql_path, 'w', encoding='utf-8') as f:
        ...
        for i, row in enumerate(rows):
            ...
            # 移除 latitude 和 longitude 字段，因为表中不存在
            ...
            sql = f"""INSERT INTO listings (address, suburb, property_type, bedrooms, bathrooms, car_spaces, land_size, price_text, price, sale_method, agent_name, agent_company, link, scraped_date, created_at) VALUES ({escape_sql(address)}, {escape_sql(suburb)}, {escape_sql(property_type)}, {bedrooms}, {bathrooms}, {car_spaces}, {land_size}, {escape_sql(price_text)}, {price}, {escape_sql(sale_method)}, {agent_name_sql}, {agent_company_sql}, {escape_sql(link)}, {escape_sql(scraped_date)}, NOW());\n"""
